# modules/hostel.py - ИСПРАВЛЕННАЯ ВЕРСИЯ (БЕЗ PLOTLY)
import os
import pandas as pd
...
st.markdown(HOSTEL_STYLES, unsafe_allow_html=True)
...
def register_fonts():
    ...
    try:
        # Сначала пробуем стандартные шрифты Windows
        font_paths = [
        ...
        # Если не нашли, ищем любой .ttf в папке Fonts
        if not registered:
...
def convert_horizontal_to_vertical(uploaded_file):
    ...
    try:
        ...
        # Ищем строку с заголовками (содержит [Вход], [Выход], [Присутствие])
        header_row_idx = None
        for idx in range(len(df_raw)):
            row = df_raw.iloc[idx]
            for val in row:
                if isinstance(val, str) and ('[Вход]' in val or '[Выход]' in val or '[Присутствие]' in val):
                    header_row_idx = idx
                    break
        ...
        for col_idx, val in enumerate(header_row):
            if isinstance(val, str) and ('Сотрудник' in val or 'сотрудник' in val):
                employee_col = col_idx
                break
        
        if employee_col is None:
            # Пробуем найти по данным в следующей строке
            for col_idx in range(len(header_row)):
                if col_idx < len(df_raw.iloc[header_row_idx + 1]):
                    val = df_raw.iloc[header_row_idx + 1, col_idx]
                    if isinstance(val, str) and val.strip() and not any(k in val for k in ['Вход', 'Выход', 'Присутствие']):
                        employee_col = col_idx
                        break
        
        if employee_col is None:
            employee_col = 1
            debug_info.append(f"⚠️ Колонка с сотрудниками не найдена, используем: {employee_col}")
        else:
...
def export_to_excel_formatted(df_vertical, df_summary, blocks_info):
    ...
    if blocks_info:
        ...
        # Если нет блоков, просто сохраняем данные
        df_vertical.to_excel(wb, sheet_name='В день', index=False)
        ...
        with tab4:
            ...
            with col2:
                ...
                if not days_count.empty:
                    # Используем st.bar_chart вместо plotly
                    st.bar_chart(days_count.set_index('Дата'))
                else:
                    st.info("Нет данных")
            ...
            if not hist_data.empty:
                # Используем st.area_chart вместо plotly
